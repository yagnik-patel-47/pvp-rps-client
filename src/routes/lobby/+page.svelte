<script lang="ts">
	...
	import { getInitialsFromFullName, socket, type Player } from "$lib";
	...
	socket.on("exchangeData", (data: Player) => {
		if (opponent) return;
		opponent = data;
		if ($gameState) {
		...
		socket.emit("finishExchangeData");
	});

	socket.on("prepareRound1", () => {
		goto("/game", { replaceState: true });
		socket.emit("lobbyToGame");
	});
</script>
...
<main>
	...
	<section class="lg:py-20 py-8 space-y-8 px-8 lg:px-20">
		{#if loading}
			<h1 class="text-2xl lg:text-3xl font-medium text-center">Loading...</h1>
		{:else}
			{#if !opponent}
